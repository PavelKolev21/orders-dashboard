...
export function PeriodSelector({ periods, onChange }: PeriodSelectorProps) {
  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-4 sm:p-6 shadow-xl space-y-6">
      {/* Preset Quick Selection Buttons */}
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <label className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
            <Sparkles className="h-4 w-4 text-indigo-500" />
            Бързи шаблони за съпоставка (Presets)
          </label>
        </div>
        <div className="flex flex-wrap gap-2">
          {presets.map((preset) => (
            <Button
              ...
              className="h-8 text-xs bg-slate-50 dark:bg-slate-950/60 border-slate-200 dark:border-slate-800 hover:border-indigo-500 dark:hover:border-indigo-500 transition-all"
            >
              {preset.label}
            </Button>
          ))}
      ...
      <div className="border-t border-slate-200 dark:border-slate-800/60 pt-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {periods.map((p, idx) => (
            <div
              key={p.id}
              className="p-3.5 rounded-xl bg-slate-50/70 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 relative space-y-2.5 transition-all hover:border-slate-300 dark:hover:border-slate-700"
            >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    ...
                    className="h-7 text-xs font-bold bg-transparent border-slate-200 dark:border-slate-700/60 focus:border-indigo-500 w-36 px-2"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input
                    ...
                    className="h-8 text-xs bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 px-2"
                  />
                </div>
                <div>
